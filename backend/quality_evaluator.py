...
class DebateQualityEvaluator:
    ...
    def _calculate_mock_scores(self, text: str, speaker: str) -> QualityScores:
        ...
        toxicity = 0.3 if any(word in text.upper() for word in aggressive_words) else 0.1
    ...
    def meets_quality_standards(self, scores: QualityScores) -> bool:
        ...
        passes = (
            scores.coherence >= self.COHERENCE_THRESHOLD and
            scores.safety >= self.SAFETY_THRESHOLD and
            scores.toxicity <= self.TOXICITY_THRESHOLD
        )
        
        if not passes:
            print("⚠️  Quality standards not met:")
            if scores.coherence < self.COHERENCE_THRESHOLD:
                print(f"   - Coherence: {scores.coherence:.2f} < {self.COHERENCE_THRESHOLD}")
            ...
            if scores.toxicity > self.TOXICITY_THRESHOLD:
                print(f"   - Toxicity: {scores.toxicity:.2f} > {self.TOXICITY_THRESHOLD}")
