...
class MetricsCollector:
    ...
    def record_debate_generated(
        self, topic: str, duration: float, success: bool = True
    ):
        """
        ...
        print(
            f"📊 METRIC: Debate generated - {topic} ({duration:.2f}s) - {'✅' if success else '❌'}"
        )
    ...
    def get_metrics_summary(self) -> Dict:
        ...
        successful_debates = sum(
            1 for m in self.metrics if m["type"] == "debate_generated" and m["success"]
        )
        ...
        for score_type in set(m["score_type"] for m in quality_scores):
            scores = [
                m["value"] for m in quality_scores if m["score_type"] == score_type
            ]
        ...
        return {
            ...
            "success_rate": (
                successful_debates / total_debates if total_debates > 0 else 0
            ),
...
class Observability:
    ...
    def log_debate_start(self, topic: str, turns: int, session_id: str):
        ...
        self.logger.info(
            "Debate started",
            extra={"topic": topic, "turns": turns, "session_id": session_id},
        )
    ...
    def log_debate_complete(self, topic: str, session_id: str, duration: float):
        ...
        self.logger.info(
            "Debate completed",
            extra={"topic": topic, "session_id": session_id, "duration": duration},
        )
    ...
    def log_turn_generated(self, speaker: str, turn: int, text: str):
        ...
        self.logger.debug(
            "Turn generated",
            extra={"speaker": speaker, "turn": turn, "text_length": len(text)},
        )
...
def test_observability():
    ...
    obs.log_debate_start(
        topic="AI vs Human Intelligence", turns=6, session_id="debate_test123"
    )
    ...
    obs.log_debate_complete(
        topic="AI vs Human Intelligence", session_id="debate_test123", duration=1.5
    )
