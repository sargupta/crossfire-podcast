...
class LoadTester:
    ...
    async def run_load_test(
        self,
        concurrent_debates: int = 10,
        total_debates: int = None
    ):
        """
        ...
        for batch_start in range(0, total_debates, concurrent_debates):
            ...
            print(f"Running batch {batch_start//concurrent_debates + 1}: debates {batch_start+1}-{batch_end}")
            
            tasks = [
                self.simulate_debate(i)
