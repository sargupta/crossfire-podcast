    ...
    async def _generate_debate(self, topic: str, turns: int) -> List[DebateTurn]:
        ...
        debaters = [
            (
                "sovereignist",
                "Sovereignist",
                "Tradition has proven itself! Change must prove worthy!",
            ),
            (
                "reformist",
                "Reformist",
                "The future waits for NO ONE. Evolve or perish!",
            ),
            (
                "technocrat",
                "Technocrat",
                "Data shows 67% failure rate. Evidence over emotion!",
            ),
...
"""
        ...
        # Save file
        safe_topic = "".join(c if c.isalnum() else "_" for c in self.debate_data.topic)[
            :40
        ]
