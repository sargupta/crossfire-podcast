...
class PodcastOrchestrator:
    ...
    def _synthesize_line(self, text: str, speaker_id: str) -> bytes:
        ...
        voice_name = self.VOICE_MAP.get(speaker_id.lower(), "en-US-Neural2-D") # Fallback
        if "sovereignist" in speaker_id: voice_name = self.VOICE_MAP["sovereignist"]
        elif "reformist" in speaker_id: voice_name = self.VOICE_MAP["reformist"]
        elif "technocrat" in speaker_id: voice_name = self.VOICE_MAP["technocrat"]
        elif "humanist" in speaker_id: voice_name = self.VOICE_MAP["humanist"]
        elif "shakti" in speaker_id: voice_name = self.VOICE_MAP["shakti"]
    ...
    def generate_cast(self, topic: str) -> tuple[List[Dict[str, str]], str]:
        ...
        candidate_models = ["gemini-2.0-flash-exp", "gemini-1.5-flash-001", "gemini-pro"]
        
        for model_name in candidate_models:
            try:
                model = GenerativeModel(model_name, system_instruction="You are the Casting Director for a high-stakes debate.")
                response = model.generate_content(self.CASTING_PROMPT.format(topic=topic))
    ...
    def generate_debate(self, topic: str, turns: int = 6) -> Dict:
        ...
        if not cast: raise ValueError("No agents could be cast.")
        ...
        for profile in cast:
            ...
            # Map keys
            key = cat_id
            if "sovereignist" in cat_id: key = "sovereignist"
            elif "reformist" in cat_id: key = "reformist"
            elif "technocrat" in cat_id: key = "technocrat"
            elif "humanist" in cat_id: key = "humanist"
            elif "shakti" in cat_id: key = "shakti"
            ...
            3. SHOW NO MERCY.
            """
            agent_model = GenerativeModel(working_model_name, system_instruction=system_prompt)
            self.agents[key] = SimpleAgentWrapper(profile['name'], agent_model)
            active_cast.append(profile)

        if not self.agents: raise ValueError("No agents instantiated.")

        # 3. Execution (Script Generation)
        script = []
        history_text = f"TOPIC: {topic}\nPANEL:\n" + "\n".join([f"- {p['name']} ({p['sub_role']})" for p in active_cast])
        
        # Host Intro
        host = self.agents.get('shakti')
        if host:
            time.sleep(5) 
            resp = host.chat.send_message(f"Start the debate. Introduce the topic '{topic}' and the panel.")
