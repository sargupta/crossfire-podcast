...
class ManagedSessionService:
    ...
    async def create_session(
        self, topic: str, participants: Optional[List[str]] = None
    ) -> DebateContext:
        """
    ...
    async def update_context(self, session_id: str, turn_data: Dict) -> DebateContext:
        """
...
async def test_managed_sessions():
    ...
    # Create session
    session = await service.create_session(topic="AI in Healthcare")
    ...
    await service.update_context(
        session.session_id,
        {"speaker": "shakti", "text": "Welcome to CROSSFIRE!", "turn": 0},
    )
