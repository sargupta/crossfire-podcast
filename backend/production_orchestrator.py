...
class ProductionADKOrchestrator:
    ...
    async def generate_debate_stream(
        ...
        # Trace entire debate
        with self.observability.tracer.span("debate_generation", topic=topic, turns=turns):
            try:
                ...
                self.observability.log_error('debate_generation_failed', str(e), session_id=session_id)
                yield {
                    'type': 'error',
                    'message': str(e),
                    'session_id': session_id
    ...
    async def _generate_intro(self, topic: str, session_id: str) -> Dict:
        ...
        with self.observability.tracer.span("intro_generation", topic=topic):
            ...
            # Update session
            await self.session_service.update_context(session_id, {
                'speaker': 'shakti',
                'text': intro_text,
                'turn': 0,
                'quality_scores': {
            ...
            # Record metrics
            self.observability.metrics.record_quality_score('coherence', scores.coherence, 'shakti')
            self.observability.metrics.record_quality_score('safety', scores.safety, 'shakti')
    ...
    async def _generate_turn(
        ...
        with self.observability.tracer.span("turn_generation", turn=turn, speaker=speaker_id):
            turn_start = datetime.now()
            
            # Get conversation history from session
            history = self.session_service.format_history_for_prompt(session_id, last_n=3)
            ...
            self.observability.metrics.record_turn_generated(speaker_id, turn, turn_duration)
            self.observability.metrics.record_quality_score('coherence', scores.coherence, speaker_id)
            self.observability.metrics.record_quality_score('safety', scores.safety, speaker_id)
    ...
    async def _generate_conclusion(self, topic: str, session_id: str) -> Dict:
        ...
        with self.observability.tracer.span("conclusion_generation"):
            conclusion_text = "What a BATTLE! Four perspectives, zero mercy. THIS is CROSSFIRE!"
            
            # Evaluate
            scores = await self.quality_evaluator.evaluate_turn(conclusion_text, 'shakti')
...
async def test_production_orchestrator():
    ...
    async for event in orchestrator.generate_debate_stream(topic, turns=4):
        ...
        if 'quality_scores' in event:
            ...
            print(f"Quality: C={scores.get('coherence', 0):.2f} S={scores.get('safety', 0):.2f}")
    ...
    print(f"   Avg Coherence: {summary['average_quality_scores'].get('coherence', 0):.2f}")
